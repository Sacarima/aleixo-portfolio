...
// --- Helpers to keep links correct in production ---
...
const ensureHttps = (url) => {
  ...
  return `https://${s.replace(/^\/+/, "")}`; // add protocol
};
const normalizeHref = (raw) => {
  let href = ensureHttps(raw);
  try {
    const u = new URL(href);
    const host = u.hostname.replace(/^www\./, "").toLowerCase();
    if (host === OWN_DOMAIN) {
      // Internal link to our own site → use relative path for Next routing
...
const projects = [
  ...
  },
  {
    ...
    live:
      "https://chromewebstore.google.com/detail/read-later-with-estinatio/cifdmmkbpnkggmebbfeaaffhjbdipamb",
  },
  {
    num: "04",
    category: "Frontend",
    ...
    title: "Project 4",
    description:
      "Alputo Trading is a dynamic trading company based in Angola and Namibia, focused on facilitating cross-border trade and promoting economic growth. Using React, I developed a responsive, user-friendly website that showcases the company’s services, including general trade, logistics, foreign trade consulting, and more.",
  ...
  },
  {
    ...
    title: "Project 6",
    description:
      "Responsive and feature-rich eCommerce platform for selling clothes, using modern web technologies and tools. For the backend, Strapi was used to manage and deliver dynamic content, providing an easy-to-use interface for content updates. The platform integrates Stripe for secure payment processing",
    stack: [
      [{ name: "HTML 5" }],
      [{ name: "CSS" }],
      [{ name: "Tailwind.css" }],
...
const Projects = () => {
  ...
  const githubHref = ensureHttps(project.github);
  const liveIsExternal = isExternal(liveHref);
      ...
      <div className="container mx-auto ">
        <div className="flex flex-col xl:flex-row xl:gap-[30px]">
          ...
          <div className="w-full xl:w-[50%] xl:h-[460px] flex flex-col xl:justify-between order-2 xl:order-none">
            ...
            <h2 className="text-[32px] font-bold leading-none text-white capitalize">
            ...
            <div className="flex items-center gap-4">
              {/* LIVE LINK: chooses <a> for external, <Link> for internal */}
              {liveIsExternal ? (
                <a
                  href={liveHref}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="Open live project (new tab)"
                >
                ...
                <Link href={liveHref} aria-label="Open live project">
                  <TooltipProvider delayDuration={100}>
                    <Tooltip>
                      <TooltipTrigger className="w-[70px] h-[70px] rounded-full bg-white/5 flex items-center justify-center group gap-4 mt-4">
                        <BsArrowUpRight className="text-white text-[40px] cursor-pointer group-hover:text-[#00FF99]" />
          ...
          <div className="w-full xl:w-[50%]">
            ...
            >
              ...
              <ProjectSlideButton
                ...
                btnStyles="bg-[#00FF99] text-[#1c1c22] text-[22px] w-[44px] h-[44px] flex justify-center items-center transition-all"
              />
            </Swiper>
          </div>
        </div>
